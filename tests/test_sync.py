...
def _cfg(tmp_path, hosts=("en.wikipedia.org",), max_edits=50, dedup=True):
    return Config(
        username="Tester", hosts=list(hosts), targets=["rec"], dedup=dedup,
        max_edits=max_edits, first_run_lookback_days=7,
        state_file=str(tmp_path / "state.json"),
        user_agent="ua", note_title_template="{title}", log_level="INFO",
    )
